...
<Thought>{`(From Mafuyu's lyrics to Kanade's song... Everyone's getting better at expressing themselves. I can't keep up—)`}</Thought>

<Thought>{`(If I'm the only one left behind...)`}</Thought>

<Thought>{`(If I can't express their songs—)`}</Thought>
...
<Thought>{`(Of COURSE I'd get that. After all, I'm gonna become an artist just like Dad—)`}</Thought>
