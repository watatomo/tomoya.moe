...
function getAllPostSlugs() {
    ...
    const postSlugs = allPostPaths.map((path) =>
        path.replace(`${POSTS_DIRECTORY}/`, "").replace(POST_EXTENSION, "")
    );
...
export function getPostFieldsBySlug(slug, fields = []) {
    const fullPath = join(POSTS_DIRECTORY, `${slug}${POST_EXTENSION}`);
    const fileContents = fs.readFileSync(fullPath, "utf8");
    const { data, content } = matter(fileContents);
    const [story, chapter] = isChapterSlug(slug) ? slug.split("/") : [];
